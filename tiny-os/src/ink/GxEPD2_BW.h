...
public:
  ...
  int getPagedPixelIndex(int16_t x, int16_t y)  {
  ...
  void drawPixelInPartialWindow(int16_t x, int16_t y, uint16_t color)
  {
    if ((x < 0) || (x >= _pw_w) || (y < 0) || (y >= _pw_h))
      return;
  ...
  bool nextPage(bool refresh_mode = true)
  {
    if (1 == _pages)
    {
      if (_using_partial_mode)
      {
        ...
        uint32_t offset = _reverse ? (HEIGHT - _pw_h) * _pw_w : 0;
    ...
    if (_using_partial_mode)
    {
      ...
      if (dest_ye > dest_ys)
      {
        ...
        uint32_t offset = _reverse ? (_page_height - (dest_ye - dest_ys)) * _pw_w : 0;
        if (!_second_phase)
      ...
      if (_current_page == _pages)
      {
        ...
        if (!_second_phase)
        {
          epd2.refresh(_pw_x, _pw_y, _pw_w, _pw_h, refresh_mode);
          if (epd2.hasFastPartialUpdate)
          {
            _second_phase = true;
            fillScreen(GxEPD_WHITE);
  ...
  void drawBitmapGS(int16_t x, int16_t y, const uint8_t bitmap[], int16_t w, int16_t h)
  {
    ...
    for (int16_t j = 0; j < h; j++)
    {
      for (int16_t i = 0; i < w; i++)
      {
        ...
        // if (!(byte & 0x80))
        // {
        drawPixel(x + i, y + j, bitmap[counter++]);
        // }
      }
    }
  }

  void drawInvertedBitmap(int16_t x, int16_t y, const uint8_t bitmap[], int16_t w, int16_t h)
  {
    // taken from Adafruit_GFX.cpp, modified
    int16_t byteWidth = (w + 7) / 8; // Bitmap scanline pad = whole byte
    uint8_t byte = 0;
...
private:
  uint8_t _buffer[(GxEPD2_Type::WIDTH)*page_height];
