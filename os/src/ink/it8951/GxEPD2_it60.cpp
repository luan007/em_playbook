...
GxEPD2_it60::GxEPD2_it60(int8_t cs, int8_t dc, int8_t rst, int8_t busy) : GxEPD2_EPD(cs, dc, rst, busy, LOW, 10000000, WIDTH, HEIGHT, panel, hasColor, hasPartialUpdate, hasFastPartialUpdate),
...
void GxEPD2_it60::writeImage(const uint8_t bitmap[], int16_t x, int16_t y, int16_t w, int16_t h, bool invert, bool mirror_y, bool pgm)
{
  ...
  int16_t wb = (w); // width bytes, bitmaps are padded
  // x -= x % 8;                                                     // byte boundary
  ...
  for (int16_t i = 0; i < h1; i++)
  {
    ...
    // for (int16_t j = 0; j < w1 / 8; j++)
    {
      ...
      uint32_t idx = mirror_y ? uint32_t(j + dx) + uint32_t((h - 1 - (i + dy))) * uint32_t(wb) : uint32_t(j + dx) + uint32_t(i + dy) * uint32_t(wb);
      if (pgm)
      {
                                 ...
                                 int16_t x, int16_t y, int16_t w, int16_t h, bool invert, bool mirror_y, bool pgm)
{
  ...
  int16_t wb_bitmap = (w_bitmap); // width bytes, bitmaps are padded
  ...
  for (int16_t i = 0; i < h1; i++)
  {
    ...
    for (int16_t j = 0; j < w1; j++)
    {
      ...
      uint32_t idx = mirror_y ? x_part + (j + dx) + uint32_t((h_bitmap - 1 - (y_part + i + dy))) * uint32_t(wb_bitmap) : x_part + j + dx + uint32_t(y_part + i + dy) * uint32_t(wb_bitmap);
      if (pgm)
      {
#if defined(__AVR) || defined(ESP8266) || defined(ESP32)
        data = pgm_read_byte(&bitmap[idx]);
...
void GxEPD2_it60::refresh(int16_t x, int16_t y, int16_t w, int16_t h, bool partial_update_mode)
{
...
void GxEPD2_it60::_waitWhileBusy2(const char *comment, uint16_t busy_time)
{
  if (_busy >= 0)
  {
    ...
    while (1)
    {
      if (DISPLAY_TERMINATION && !PROTECTED)
      {
        DISPLAY_TERMINATION = 0;
        TERMINATED = 1;
        vTaskDelete(NULL);
        return;
      }
...
void GxEPD2_it60::beginWriteNative(int16_t x, int16_t y, int16_t w, int16_t h)
{
  if (_initial_write)
    writeScreenBuffer();                                          // initial full screen buffer clean
  yield();                                                        // yield() to avoid WDT on ESP8266 and ESP32
...
void GxEPD2_it60::writeChunk(const uint8_t *data1, int16_t len)
{
  for (int16_t i = 0; i < len; i++)
  {
    SPI.transfer(data1[i]);
    // if (i % 100 == 0)
    // {
    //   yield();
    // }
...
void GxEPD2_it60::writeChunkRGB(const uint16_t *data1, int16_t len)
{
  for (int16_t i = 0; i < len; i++)
  {
    SPI.transfer((uint8_t)(data1[i] & 0xff));
    // if (i % 100 == 0)
    // {
    //   yield();
    // }
...
void GxEPD2_it60::writeSingleByte(uint8_t b)
{
  SPI.transfer(b);
}

void GxEPD2_it60::endWriteNative(int16_t x, int16_t y, int16_t w, int16_t h)
{
  if (_cs >= 0)
    digitalWrite(_cs, HIGH);
  SPI.endTransaction();
...
void GxEPD2_it60::endWriteNativeNoRefresh()
{
  if (_cs >= 0)
    digitalWrite(_cs, HIGH);
  SPI.endTransaction();
