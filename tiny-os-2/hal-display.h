...
int _graphics_prev_commit = 0;

TaskHandle_t render_thread;

int render_running = 0;
#define SLEEP_BITMASK_DISPLAY 2

void HAL_DRAW_IMG(void *pvParameters)
{
    (void)pvParameters;
    render_running = 1;
    display_dbg_print_no_fullscreen(String("HELLO WORLD") + millis());
    render_running = 0;
...
int display_render_loop()
{
    if (_graphics_prev_commit != _graphics_commit)
    {
        if (e_ink_state != 1)
        {
            display_power(1); //init first
        }
        //abort render immediately
        if (display.epd2.TERMINATED != 0)
        {
            display.epd2.DISPLAY_TERMINATION = 1;
        }
        else
        {
            display.epd2.DISPLAY_TERMINATION = 0;
            display.epd2.TERMINATED = 0;
            render_running = 0;
        }
        if (display.epd2.DISPLAY_TERMINATION == 0)
        {
            render_running = 1;
            display.epd2.TERMINATED = 0; //start draw
            xTaskCreatePinnedToCore(
                HAL_DRAW_IMG, "HAL_DRAW_IMG",
                10240, NULL, 2, &render_thread, CPU);
